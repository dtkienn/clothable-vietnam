...
(function() {
	...
	function buildPrice(data, onSale, priceVaries) {
		var priceHtml = '',
			...
			salePercentCal = 100 - (data.price_min / data.compare_at_price_min * 100)
			salePercent =  onSale ? Math.ceil(salePercentCal) : 0;
		...
		priceHtml += boostPFSConfig.label.sale + ' -' + salePercent + '%</span></span>';
	...
	ProductSorting.prototype.bindEvents = function() {
		jQ('body').on('click', '.boost-pfs-filter-custom-sorting a', function(e) {
			e.preventDefault();
			var dataSort = jQ(this).attr('data-sort');
				
			FilterApi.setParam('sort', dataSort);
			FilterApi.setParam('page', 1);
			FilterApi.applyFilter('sort');
		});

		jQ('.boost-pfs-filter-custom-sorting > label').click(function() {
			jQ(this).parent().toggleClass('boost-pfs-filter-sort-active');
		});

		jQ(Selector.filterTreeMobileButton).click(function() {
			jQ('.boost-pfs-filter-custom-sorting').removeClass('boost-pfs-filter-sort-active');
		});
	};
	...
	ProductList.prototype.afterRender = function(data) {
		...
		if (Globals.queryParams.display == 'list') {
			...
			productSelector.removeClass('grid grid--view-items');
		} else {
			if (productSelector.children('.list-view-items').length > 0) {
				productSelector.children('.list-view-items').children().unwrap();
			}
			productSelector.addClass('grid grid--view-items');
		}
	}
	...
	Filter.prototype.afterRender = function(data) {
		if (!data) data = this.data;
		// Add class when collapse
		jQ('body').on('click', '.boost-pfs-filter-option-title-heading', function() {
			if(!jQ(this).hasClass('active')) {
				jQ('.boost-pfs-filter-option-title-heading').removeClass('active');
				jQ(this).addClass('active');
			}
		});

		// Add reset button to bottom of filter tree
		setTimeout(function(){
			if(jQ('.boost-pfs-filter-mobile-footer .boost-pfs-filter-mobile-toolbar-bottom').length == 0){
				jQ('.boost-pfs-filter-mobile-footer').append('<div class="boost-pfs-filter-mobile-toolbar-bottom"><button aria-label="Clear all filters" class="trigger-clear-all">Clear</button></div>');
			}
		}, 100);

		// Trigger click reset button
		jQ('body').on('click', '.trigger-clear-all', function() {
			if($('.boost-pfs-filter-mobile-toolbar-right .boost-pfs-filter-clear').length > 0) {
				$('.boost-pfs-filter-mobile-toolbar-right .boost-pfs-filter-clear').trigger('click');
			}
		});

		// Append section image with text
		if(jQ('.boost-pfs-filter-refine-by').length == 0) {
			if ($('.collection-image-with-text').length > 0 && $('.boost-pfs-filter-products .collection-image-with-text').length == 0) {
				let sectionClone = $('.collection-image-with-text').clone();
				if ($('.boost-pfs-filter-products > div').length < 8) {
					$('.boost-pfs-filter-products').append(sectionClone);
				} else {
					$('.boost-pfs-filter-products > div:nth-child(8)').after(sectionClone);
				}
			}
		}
	}
	...
	FilterTreeStyle2.prototype.getHeaderTemplate = function() {
		return `
			<div class="boost-pfs-filter-mobile-toolbar">
				<div class="boost-pfs-filter-mobile-toolbar-items">
					<div class="boost-pfs-filter-mobile-toolbar-left">
						<a href="javascript:;" class="{{class.closeFilterButton}} trigger-close-filter">
							<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
								<path d="M15 6L9 12L15 18" stroke="#212121" stroke-width="2"/>
							</svg>
						</a>
					</div>
					<div class="boost-pfs-filter-mobile-middle">{{label.refineMobile}}</div>
